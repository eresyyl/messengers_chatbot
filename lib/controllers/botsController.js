...
botsController.list = async (ctx, next) => {
	...
	try{
		const bots = await Bots.find(
			{}, 
			'name description image_url bot_link', 
			{
...
botsController.getByOrder = async (ctx, next) => {
	...
	try{
		const bot = await Bots.findOne(
			{}, 
			'post_id name description image_url more_link', 
			{
...
botsController.getReviews = async (ctx, next) => {
	...
	try{
		...
		const messages = [];
		if( bot.reviews.length == 0 ) messages.push("There is no reviews on this bot");
		else {
			let average_rate = "";
			for(let j = 0; j < bot.average_rate; j++)
				average_rate += "⭐";
			messages.push(`Average rating for "${bot.name}":\n ${average_rate}`);
			let i = 0;
			bot.reviews.forEach( (review) => {
				let review_text = `${review.user_name}\n`;
				review_text = `Rating: ${review.rate}\n\n`;
